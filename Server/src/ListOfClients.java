...
public class ListOfClients {
	...
	public String getListOfClients(){
		...
		for (int i=0;i< alias.size();i++){
			listOfclients=listOfclients.concat("<div><div>"+idusers.get(i).toString()+"</div>");
			listOfclients=listOfclients.concat("<div>"+ (String) alias.get(i)+"</div>");
			listOfclients=listOfclients.concat("<div>"+(String) email.get(i)+"</div>");
			listOfclients=listOfclients.concat("<div>"+(String) registrationDate.get(i)+"</div></div>");
